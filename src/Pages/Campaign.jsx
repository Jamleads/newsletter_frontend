      ...
      {form ? (
        ...
        <div className="flex flex-col gap-10">
          ...
          <div className="grid grid-cols-3 gap-5 text-primary-mainBlue">
            <BoardCard name={"Campaign"} total={campaignData?.length + 300} />
            <BoardCard name={"Active"} total={campaignData?.length + 100} />
            <BoardCard name={"Draft"} total={campaignData?.length} />
          </div>

          <div className="bg-white shadow-xl">
            <div className="px-5 py-2 text-primary-mainBlue flex items-center justify-between">
              <p>Total campaign ({campaignData?.length})</p>
            </div>
              ...
              <tbody>
                {campaignData?.map((item, index) => (
                  ...
                  >
                    ...
                    <td className={`${tData}`}>{item?.campaignTitle}</td>
                    <td className={`${tData}`}>{item?.campaignStatus}</td>
                    <td className={`${tData}`}>{item?.createdBy}</td>
                    <td className={`${tData}`}>{item?.createdDate}</td>
                    <td className={`${tData}`}>{item?.lastModifiedDate}</td>
                  </tr>
                ))}
              </tbody>
            </table>
